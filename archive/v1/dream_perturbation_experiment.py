"""
dream_perturbation_experiment.py
=================================
Full "Dream Perturbation" experiment for DIAMOND world models.

This script:
  1. Loads a pretrained DIAMOND Breakout checkpoint from HuggingFace
  2. Creates 5 PerturbedWorldModelEnv dream variants
  3. Trains 3 actor-critic agents:
  ...
  4. Evaluates all 3 agents in the REAL Atari (Breakout) environment
  5. Evaluates all 3 agents in HELD-OUT dream variants
  6. Saves JSON results + generates comparison figures

Run from inside the DIAMOND repository root:
    python path/to/dream_perturbation_experiment.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Dream Perturbation experiment for DIAMOND world models"
    )
    p.add_argument(
        "--diamond-root",
        type=str,
    ...
    p.add_argument(
        ...
        help="Path to agent checkpoint .pt file. "
             "If not given, downloads Breakout from HuggingFace.",
    )
    p.add_argument(
        "--output-dir",
        type=str,
    ...
    p.add_argument(
        ...
        type=int,
        default=50,
        help="Number of training epochs per agent (default: 50).",
    )
    p.add_argument(
        "--eval-episodes",
        type=int,
        default=10,
        help="Real-env evaluation episodes per agent (default: 10).",
    )
    p.add_argument(
        "--wm-horizon",
        type=int,
    ...
    p.add_argument(
        "--fast",
        action="store_true",
        help="Quick smoke-test mode: 5 epochs, 50 steps/epoch, 3 eval episodes.",
    )
    return p.parse_args()
...
def generate_comparison_figure(
    ...
    """
    Bar chart: mean real-env return for each of the 3 agents.
    Error bars = std across evaluation episodes.
    """
    agent_names = ["baseline", "single_perturb", "multi_dream"]
    display_names = ["Baseline\n(normal dream)", "Single-Perturb\n(mirrored)", "Multi-Dream\n(all 5 variants)"]
    ...
    ax.set_title("Dream Perturbation: Agent Robustness in Real Breakout\n(higher = better)", fontsize=13)
...
def generate_robustness_heatmap(
    ...
    """
    Heatmap: rows = agents (3), columns = dream variants (5).
    Cell value = mean return when that agent is evaluated in that variant.
    """
    agent_names = ["baseline", "single_perturb", "multi_dream"]
    display_agent_names = ["Baseline", "Single-Perturb", "Multi-Dream"]
...
def generate_training_curve_figure(
    ...
    """Line plot of training loss (policy entropy) per epoch for all 3 agents."""
    fig, ax = plt.subplots(figsize=(9, 4))
    colors = {"baseline": "#4C72B0", "single_perturb": "#DD8452", "multi_dream": "#55A868"}
    labels = {"baseline": "Baseline", "single_perturb": "Single-Perturb", "multi_dream": "Multi-Dream"}
...
def train_agent(
    ...
    training_mode: str,  # "baseline" | "single_perturb" | "multi_dream"
    num_epochs: int,
    steps_per_epoch: int,
    device: torch.device,
    args,
    ...
    """
    Train an actor_critic in dreams, return the trained actor_critic
    and a list of per-epoch policy entropy values.
    ...
    for epoch in range(num_epochs):
        # Pick the dream environment for this epoch (round-robin for multi_dream)
...
def main() -> None:
    ...
    if args.checkpoint:
        ...
        path_ckpt = download_pretrained_checkpoint("Breakout")
        agent_cfg_path, env_cfg_path = download_pretrained_configs()
    ...
    for training_mode, description in training_jobs:
        ...
        trained_agent, entropy_log = train_agent(
    ...
    # -----------------------------------------------------------------------
    # 7. Evaluate all 3 agents in the REAL Atari environment
    # -----------------------------------------------------------------------
    print(f"\n{'='*60}")
    ...
    # -----------------------------------------------------------------------
    # 8. Evaluate all 3 agents in HELD-OUT dream variants
    # -----------------------------------------------------------------------
    print(f"\n{'='*60}")
    ...
    for training_mode, _ in training_jobs:
        ...
        for preset_name in PRESET_NAMES:
            print(f"  Agent={training_mode:15s} | Variant={preset_name}")
    ...
    # 9. Save results JSON
    # -----------------------------------------------------------------------
    results_path = output_dir / "dream_perturbation_results.json"
    ...
    generate_comparison_figure(
        results=results,
        output_path=figures_dir / "dream_perturbation_comparison.png",
    )

    generate_robustness_heatmap(
        results=results,
        output_path=figures_dir / "dream_robustness.png",
    ...
    for training_mode, desc in training_jobs:
        ...
        print(f"{desc:<22} {np.mean(variant_returns):>20.2f}")
