"""
dream_perturbation_experiment_v4.py
====================================
Improved "Dream Perturbation" experiment for DIAMOND world models — v4.

Changes from v3:
  - Default --num-epochs raised from 200 to 500
  - Updated preset names: "noisy" → "sigma_perturb", "shifted_physics" → "adversarial_action"
  - Adds OOD cross-game evaluation: Breakout agents evaluated inside Pong's world model
    (only runs when --game Breakout; skipped silently for other games)
  - Generates ood_evaluation.png figure
  - Prints OOD summary table
...
This script:
  ...
  8. [Breakout only] OOD evaluation: runs Breakout agents inside Pong's world model
  9. Saves JSON results + generates comparison figures

Run from inside the DIAMOND repository root:
    python path/to/dream_perturbation_experiment_v4.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Dream Perturbation experiment v4 for DIAMOND world models"
    )
    p.add_argument(
        "--diamond-root",
        type=str,
    ...
    p.add_argument(
        ...
        type=int,
        default=500,
        help="Number of training epochs per agent (default: 500).",
    )
    p.add_argument(
        "--eval-episodes",
        type=int,
...
def generate_ood_evaluation_figure(
    results: Dict,
    output_path: Path,
) -> None:
    """
    Bar chart for OOD cross-game evaluation:
    4 bars, one per agent, showing mean return when Breakout agents run
    inside the Pong world model.  Higher = better cross-game generalisation.
    """
    agent_names = ["baseline", "single_perturb", "multi_dream", "adaptive_multi_dream"]
    display_names = [
        "Baseline\n(normal dream)",
    ...
    ood_data = results.get("ood_eval", {})
    means = [ood_data.get(a, {}).get("mean_return", 0.0) for a in agent_names]
    stds = [ood_data.get(a, {}).get("std_return", 0.0) for a in agent_names]

    fig, ax = plt.subplots(figsize=(10, 5))
    x = np.arange(len(agent_names))
    bars = ax.bar(
        x, means, yerr=stds, capsize=8, color=colors,
        alpha=0.88, edgecolor="black", linewidth=0.8,
        error_kw={"elinewidth": 2, "ecolor": "dimgray"},
    )
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(display_names, fontsize=11)
    ax.set_ylabel("Mean Episode Return (Pong World Model)", fontsize=12)
    ax.set_title(
        "OOD Cross-Game: Breakout Agents → Pong World Model\n"
        "(higher = better cross-game generalisation)",
        fontsize=13,
    )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    fig.text(
        0.01, 0.01,
        "Breakout actor-critics (4-action output) zero-padded to Pong's 6-action space.",
        fontsize=8, color="gray", va="bottom",
    )
    ...
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Figure] Saved OOD evaluation chart: {output_path}")


# ---------------------------------------------------------------------------
# OOD cross-game evaluation (Breakout → Pong)
# ---------------------------------------------------------------------------

def run_ood_evaluation(
    trained_agents: Dict,
    agent_display_names: Dict[str, str],
    training_jobs: List[Tuple[str, str]],
    diamond_root: str,
    dataset_dir: Path,
    batch_size: int,
    wm_horizon: int,
    ...
    device: torch.device,
    num_episodes: int = 20,
    init_collect_steps: int = 500,
) -> Dict[str, Dict]:
    """
    OOD Evaluation: Breakout agents evaluated inside a Pong world model.

    This tests whether dream-perturbation training produces more generalisable
    representations by running Breakout policies in an entirely different game's
    world model.

    Steps:
      1. Download the Pong pretrained DIAMOND checkpoint from HuggingFace.
      2. Instantiate a Pong DIAMOND agent.
      3. Build a Pong WorldModelEnv (requires a small Pong dataset for burn-in).
      4. For each Breakout actor_critic:
           - Roll out in the Pong WorldModelEnv for `num_episodes` episodes.
           - The Breakout actor_critic produces 4-action logits; Pong has 6 actions.
             We zero-pad: padded_logits = F.pad(logits, (0, 6 - 4)).
           - Record mean/std return.

    Returns
    -------
    dict: {agent_name: {"mean_return": float, "std_return": float}}
    """
    from torch.distributions.categorical import Categorical
    from huggingface_hub import hf_hub_download
    from omegaconf import OmegaConf
    ...
    lff_src = str(Path(diamond_root).parent / "learning-from-failure" / "src")
    if lff_src not in sys.path:
        sys.path.insert(0, lff_src)

    print(f"\n{'#'*60}")
    print("# OOD Evaluation: Breakout agents in Pong world model")
    print(f"{'#'*60}")

    # ------------------------------------------------------------------
    # 1. Download Pong checkpoint and configs
    # ------------------------------------------------------------------
    print("[OOD] Downloading Pong checkpoint from HuggingFace…")
    pong_ckpt_path = hf_hub_download(
        repo_id="eloialonso/diamond",
        filename="atari_100k/models/Pong.pt",
    )
    agent_cfg_path = hf_hub_download(
        repo_id="eloialonso/diamond",
        filename="atari_100k/config/agent/default.yaml",
    ...
    print(f"[OOD] Pong checkpoint: {pong_ckpt_path}")

    # ------------------------------------------------------------------
    # 2. Load Pong configs and instantiate Pong agent
    # ------------------------------------------------------------------
    agent_cfg_raw = OmegaConf.load(agent_cfg_path)
    env_cfg_raw = OmegaConf.load(env_cfg_path)

    env_cfg_raw.train.id = "PongNoFrameskip-v4"
    env_cfg_raw.test.id = "PongNoFrameskip-v4"

    img_channels = int(agent_cfg_raw.denoiser.inner_model.img_channels)
    img_size = int(env_cfg_raw.train.size)
    ...
    # Build a minimal Pong real env to determine num_actions
    pong_train_env = make_atari_env(num_envs=1, device=device, **env_cfg_raw.train)
    num_pong_actions = int(pong_train_env.num_actions)
    print(f"[OOD] Pong num_actions = {num_pong_actions}")

    agent_cfg = instantiate(agent_cfg_raw, num_actions=num_pong_actions)
    pong_agent = Agent(agent_cfg).to(device).eval()
    pong_agent.load(Path(pong_ckpt_path))
    print("[OOD] Pong agent checkpoint loaded")

    # ------------------------------------------------------------------
    # 3. Build a small Pong dataset for world-model burn-in
    # ------------------------------------------------------------------
    pong_dataset_dir = dataset_dir.parent / "pong_ood_dataset" / "train"
    pong_dataset = Dataset(pong_dataset_dir, "pong_ood_dataset", cache_in_ram=False)
    pong_dataset.load_from_default_path()

    if pong_dataset.num_steps < init_collect_steps:
        print(f"[OOD] Collecting {init_collect_steps} Pong steps for burn-in…")
        from coroutines.collector import make_collector, NumToCollect
        collector = make_collector(
            pong_train_env, pong_agent.actor_critic, pong_dataset, epsilon=1.0
        )
        collector.send(NumToCollect(steps=init_collect_steps))
        pong_dataset.save_to_default_path()
        print(f"[OOD] Pong dataset: {pong_dataset.num_episodes} eps, {pong_dataset.num_steps} steps")

    # ------------------------------------------------------------------
    # 4. Build Pong WorldModelEnv
    # ------------------------------------------------------------------
    use_cuda = (device.type == "cuda")
    make_data_loader = partial(
        DataLoader,
        dataset=pong_dataset,
        collate_fn=collate_segments_to_batch,
        num_workers=0,
        persistent_workers=False,
        pin_memory=use_cuda,
    ...
    pong_bs = BatchSampler(
        pong_dataset, rank=0, world_size=1,
        batch_size=batch_size, seq_length=num_steps_conditioning,
    )
    pong_dl = make_data_loader(batch_sampler=pong_bs)

    pong_diffusion_cfg = DiffusionSamplerConfig(num_steps_denoising=3)
    pong_wm_cfg = WorldModelEnvConfig(
        horizon=wm_horizon,
        num_batches_to_preload=3,
        diffusion_sampler=pong_diffusion_cfg,
    )
    pong_wm_env = WorldModelEnv(
        denoiser=pong_agent.denoiser,
        rew_end_model=pong_agent.rew_end_model,
        data_loader=pong_dl,
        cfg=pong_wm_cfg,
        return_denoising_trajectory=False,
    )
    print("[OOD] Pong WorldModelEnv built")

    # ------------------------------------------------------------------
    # 5. Evaluate each Breakout actor_critic in the Pong world model
    # ------------------------------------------------------------------
    ood_results: Dict[str, Dict] = {}

    for training_mode, description in training_jobs:
        print(f"\n[OOD] Evaluating: {description}")
        breakout_ac = trained_agents[training_mode].actor_critic
        breakout_ac.eval()

        returns = []
        num_envs = pong_wm_env.num_envs
        lstm_dim = breakout_ac.lstm_dim

        hx = torch.zeros(num_envs, lstm_dim, device=device)
        cx = torch.zeros(num_envs, lstm_dim, device=device)

        obs, _ = pong_wm_env.reset()
        ep_returns = torch.zeros(num_envs, device=device)
        episodes_done = 0
        max_steps = num_episodes * 300
        ...
        for _ in range(max_steps):
            ...
            with torch.no_grad():
                # Breakout actor_critic produces logits over 4 actions.
                # Pong has 6 actions. Zero-pad the logits before taking argmax.
                logits_4, val, (hx, cx) = breakout_ac.predict_act_value(obs, (hx, cx))

            # Zero-pad: [B, 4] → [B, 6]
            num_breakout_actions = logits_4.shape[-1]
            if num_breakout_actions < num_pong_actions:
                padded_logits = F.pad(
                    logits_4, (0, num_pong_actions - num_breakout_actions)
                )
            else:
                padded_logits = logits_4[:, :num_pong_actions]

            act = Categorical(logits=padded_logits).sample()

            obs, rew, end, trunc, info = pong_wm_env.step(act)
            ep_returns += rew
        ...
        mean_r = float(np.mean(returns[:num_episodes]))
        std_r = float(np.std(returns[:num_episodes]))
        ood_results[training_mode] = {
            "mean_return": mean_r,
            "std_return": std_r,
            "num_episodes": min(len(returns), num_episodes),
        }
        print(
            f"[OOD] {description}: "
            f"mean_return={mean_r:.2f} ± {std_r:.2f} "
            f"over {ood_results[training_mode]['num_episodes']} episodes"
        )
...
PRESET_NAMES = ["normal", "mirrored", "sigma_perturb", "adversarial_action", "hard_mode"]
...
def train_agent(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % max(1, num_epochs // 5) == 0:
            print(
                f"  Epoch {epoch+1:3d}/{num_epochs} | preset={preset_name:25s} | "
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
    # 10. OOD Cross-Game Evaluation (Breakout only)
    # -----------------------------------------------------------------------
    if game_name == "Breakout":
        ...
        ood_eval_episodes = max(5, 20) if not args.fast else 5
        ...
        try:
            ood_results = run_ood_evaluation(
                ...
                diamond_root=args.diamond_root,
                ...
                init_collect_steps=max(200, args.init_collect_steps // 2),
            )
            results["ood_eval"] = ood_results
        except Exception as exc:
            print(f"[OOD] WARNING: OOD evaluation failed: {exc}")
            print("[OOD] Continuing without OOD results.")
            results["ood_eval"] = {}
    else:
        print(f"\n[OOD] Skipping OOD evaluation (game={game_name!r}, only runs for Breakout)")

    # -----------------------------------------------------------------------
    # 11. Save results JSON
    # -----------------------------------------------------------------------
    results_path = output_dir / "dream_perturbation_results_v4.json"
    ...
    generate_comparison_figure(
        results=results,
        output_path=figures_dir / "dream_perturbation_comparison_v4.png",
    )

    generate_robustness_heatmap(
        results=results,
        output_path=figures_dir / "dream_robustness_v4.png",
        preset_names=PRESET_NAMES,
    )

    generate_training_curve_figure(
        training_logs=results["training_logs"],
        output_path=figures_dir / "training_curves_v4.png",
    )

    generate_transfer_robustness_figure(
        results=results,
        output_path=figures_dir / "transfer_robustness_v4.png",
    )

    if results.get("ood_eval"):
        generate_ood_evaluation_figure(
            results=results,
            output_path=figures_dir / "ood_evaluation.png",
        )
    ...
    # OOD summary (Breakout only)
    if results.get("ood_eval"):
        print(f"\n{'='*60}")
        print("SUMMARY — OOD: Breakout agents in Pong world model")
        print(f"{'='*60}")
        print("(higher = better cross-game generalisation)")
        print("-" * 60)
        for training_mode, desc in training_jobs:
            ood = results["ood_eval"].get(training_mode, {})
            mean_r = ood.get("mean_return", float("nan"))
            std_r = ood.get("std_return", float("nan"))
            print(f"{agent_display_names[training_mode]:<28} {mean_r:>12.2f} ± {std_r:.2f}")
