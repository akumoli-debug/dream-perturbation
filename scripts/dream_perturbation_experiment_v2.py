"""
dream_perturbation_experiment_v2.py
====================================
Improved "Dream Perturbation" experiment for DIAMOND world models.
...
This script:
  ...
  4. Evaluates all 4 agents in the REAL Atari environment
  5. Evaluates all 4 agents in HELD-OUT dream variants
  6. Saves JSON results + generates comparison figures

Run from inside the DIAMOND repository root:
    python path/to/dream_perturbation_experiment_v2.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Dream Perturbation experiment v2 for DIAMOND world models"
    )
    p.add_argument(
        "--diamond-root",
        type=str,
...
def main() -> None:
    ...
    # 9. Save results JSON
    # -----------------------------------------------------------------------
    results_path = output_dir / "dream_perturbation_results_v2.json"
    ...
    generate_comparison_figure(
        results=results,
        output_path=figures_dir / "dream_perturbation_comparison_v2.png",
    )

    generate_robustness_heatmap(
        results=results,
        output_path=figures_dir / "dream_robustness_v2.png",
        preset_names=PRESET_NAMES,
    )

    generate_training_curve_figure(
        training_logs=results["training_logs"],
        output_path=figures_dir / "training_curves_v2.png",
    )
